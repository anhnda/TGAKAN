...
Device defaults to **GPU (cuda) when available**, else CPU. Override with
`device='cpu'` in TGAKANSurrogate(...), or it auto-falls-back if no GPU is found.
...
## Notes
- GPU (cuda) by default; set device='cpu' in TGAKANSurrogate(...) to force CPU.
